...
# OR-Intern stubs: No HF sandbox/jobs infrastructure
CPU_FLAVORS = ["cpu-basic", "cpu-upgrade"]
DEFAULT_CPU_SANDBOX_HARDWARE = "cpu-basic"

async def start_cpu_sandbox_preload(session):  # noqa: no-op stub
    pass

async def teardown_session_sandbox(session):  # noqa: no-op stub
    pass
...
async def _cleanup_on_cancel(session: Session) -> None:
    """Cancel running solve jobs and cleanup when the user interrupts."""
    # OR-Intern: Cancel running solver processes
    solver_processes = getattr(session, "_running_solver_processes", None)
    if solver_processes:
        for proc in list(solver_processes):
            try:
                proc.terminate()
                logger.info("Terminated solver process on cancel")
            except Exception as e:
                logger.warning("Failed to terminate solver process: %s", e)
        solver_processes.clear()
            ...
            try:
                ...
                if approval_required_tools:
                    ...
                    for tc, tool_name, tool_args, decision in approval_required_tools:
                        # OR-Intern: No sandbox script resolution needed.
                        # Future: resolve model file paths for solve_job here.
