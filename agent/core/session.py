...
class Session:
    ...
    def save_and_upload_detached(self, repo_id: str = "") -> Optional[str]:
        ...
        OR-Intern: In Phase 0, only saves locally. Upload to remote
        storage will be added in future phases.

        Args:
            repo_id: (unused, kept for API compatibility)

        Returns:
            Path to local save file
        """
        return self.save_trajectory_local(upload_status="completed")
