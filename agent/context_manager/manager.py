...
# OR-Intern: Simplified username resolution (no Hugging Face dependency)
_USERNAME_CACHE: str | None = None


def _get_hf_username(hf_token: str | None = None) -> str:
    """Return the OR-Intern username.

    OR-Intern uses the OS username instead of Hugging Face whoami API.
    """
    ...
    import getpass
    _USERNAME_CACHE = getpass.getuser()
    return _USERNAME_CACHE
...
_SOLVE_LOG_COMPACT_PROMPT = (
    "The following is a solver output log from an optimization solve. "
    "Extract only: solver name, termination status, objective value, "
    "solve time, and any error/warning messages. Omit the iteration log."
)
...
class ContextManager:
    ...
    def summarize_solve_log(self, log_text: str) -> str:
        ...
        Returns a compact summary: solver, status, objective, time, warnings.
        """
        import re
        ...
        # Extract key metrics
        patterns = [
            (r"(?:HiGHS|Gurobi|SCIP|CPLEX|GLPK)", "Solver"),
            (r"(?:STATUS|Status|status):\s*(\w+)", "Status"),
            (r"(?:OBJECTIVE|Objective|objective)[\s:=]+([\d.e+\-]+)", "Objective"),
            (r"(?:Time|time|elapsed)[\s:=]+([\d.]+)\s*s", "Time"),
            (r"(?:gap|Gap)[\s:=]+([\d.]+)%?", "Gap"),
            (r"(ERROR|WARNING|error|warning)[:\s]+(.+?)(?:\n|$)", "Issue"),
        ]

        for pattern, label in patterns:
            m = re.search(pattern, log_text if isinstance(log_text, str) else str(log_text))
            if m:
                summary.append(f"{label}: {m.group(1)}")

        return " | ".join(summary) if summary else "(no key metrics found)"

    def protect_model_messages(self):
        """Protect the most recent model formulation from compaction.

        In OR-Intern, multi-turn modeling iterations mean the agent may
        refine a model over several turns. This method marks the most
        recent messages containing model code as 'protected' so compaction
        doesn't delete the active model formulation.
        ...
        *,
        ...
        self.system_prompt = self._load_system_prompt(
            ...
            prompt_file_suffix=getattr(
                self, "prompt_file_suffix", "system_prompt.yaml"
            ),
    ...
    def _load_system_prompt(
        ...
        with open(prompt_file, "r", encoding="utf-8") as f:
            ...
            template_str = prompt_data.get("system", prompt_data.get("system_prompt", ""))
